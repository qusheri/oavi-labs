...
В данной работе после нормализации использовался порог:

```text
T = 30
```

Пиксели, для которых `G >= T`, считались контуром. В бинарном результате контуры показаны чёрным цветом на белом фоне, чтобы они были хорошо видны в отчёте.
